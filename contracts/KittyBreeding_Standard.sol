...
contract KittyBreeding_Standard
{
    struct KittyInfo
    {
        bool gender;
    ...
    function registerNewBorns() public
    {
        ...
        {
            _registerNewBorns(newBorns);
            delete newBorns;
        }
    }
    
    function _registerNewBorns(KittyInfo[] memory new_borns) internal
    {
        for (uint i = 0; i < new_borns.length; i++)
        {
            uint id = allKitties.length;
            allKitties.push(new_borns[i]);
            _refreshKitty(new_borns[i].owner, i, id);
        }
    }
    
    function _refreshKitty(address _scope, uint newBornIndex, uint id) internal
    {
